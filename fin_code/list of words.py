...
def F_articulation_manner(word):
    ipa_consonants = ['dz', 'ts', 'tʃ', 'dʒ', 'R', 'r', 'l', 'lj', 'm', 'n', 'nj',
                      'b', 'p', 'd', 't', 'ɟ', 'c', 'ɡ', 'k',
    ...
    ipa_manner = ['A', 'A', 'A', 'A', 'T', 'T', 'L', 'L', 'N', 'N', 'N',
                  'P', 'P', 'P', 'P', 'P', 'P', 'P', 'P',
                  'F', 'F', 'F', 'F', 'F', 'F', 'F', 'G'] #ДОБАВЛЕНО 'R'

    for element in word:
        for i in range(0, 27):                              #25): — это без р
            if ipa_consonants[i] == element:
...
def M_create_table_1():
    ...
    asd = my_lines[9102:20103]       # тестовая выборка
